...
`phantom` is the previous-map high score shown as the group's ghost, `total` is
the summarized promotion bar, and `history` stores finished-map highs by seed.
...
map high is folded into the total with `total * 0.25 + seedHigh * 0.75`.
