...
- [ ] update obstacles to sometimes be circles sometimes walls that can be +/-45 deg or 90 deg (perpendicular to the road). A rect cannot can be any width but cannot exceed the current width of the road minus one lane. ie: can be 2x lane width on a 3way road, but only 1 length on a 2 lane road. The higher the seed of the road is
