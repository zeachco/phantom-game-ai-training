...
export function drawLevel(
  ...
  for (let i = 0; i < inputs.length; i++) {
    for (let j = 0; j < outputs.length; j++) {
      ...
      ctx.lineTo(getNodeX(outputs, j, left, right), top + RADIUS);
      ctx.lineWidth = 2;
      ctx.strokeStyle = getColor(weights[i][j]);
      getColor(weights[i][j]);
      ctx.stroke();
    }
  }
  ...
  for (let i = 0; i < inputs.length; i++) {
    ...
    ctx.fillStyle = getColor(inputs[i]);
  ...
  for (let i = 0; i < outputs.length; i++) {
    ...
    ctx.fillStyle = getColor(outputs[i]);
    ctx.stroke();
