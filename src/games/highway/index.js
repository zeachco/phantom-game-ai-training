import { loadModel } from "../../ai/utils";
...
export default async (state) => {
    ...
    function initialize() {
        ...
        window.bestBrain = loadModel("highway");
        if (model) {
        ...
        state.traffic.push(
        // trial of evasion
        new Car(road.getLaneCenter(0), -300, 30, 50, ControlType.DUMMY, 0.1), new Car(road.getLaneCenter(1), -600, 30, 50, ControlType.DUMMY, 0.2), new Car(road.getLaneCenter(2), -300, 30, 50, ControlType.DUMMY, 0.3), 
        // moving cars
        new Car(road.getLaneCenter(0), -400, 30, 50, ControlType.DUMMY, 2), new Car(road.getLaneCenter(0), -600, 30, 50, ControlType.DUMMY, 2), 
        // new Car(road.getLaneCenter(1), -400, 30, 50, ControlType.DUMMY, 2),
        new Car(road.getLaneCenter(2), -300, 30, 50, ControlType.DUMMY, 2), new Car(road.getLaneCenter(1), -500, 30, 50, ControlType.DUMMY, 2), new Car(road.getLaneCenter(1), -900, 30, 50, ControlType.DUMMY, 2), new Car(road.getLaneCenter(2), -700, 30, 50, ControlType.DUMMY, 2), 
        // fail wall
        new Car(road.getLaneCenter(0), 400, 30, 50, ControlType.DUMMY, 2.6), new Car(road.getLaneCenter(1), 375, 30, 50, ControlType.DUMMY, 2.6), new Car(road.getLaneCenter(2), 400, 30, 50, ControlType.DUMMY, 2.6));
        for (let i = 3; i < lanes; i++) {
            state.traffic.push(new Car(road.getLaneCenter(i), 300, 30, 50, ControlType.DUMMY, 2.5));
        }
        // traffic.push(
