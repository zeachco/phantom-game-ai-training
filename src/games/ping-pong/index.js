    ...
    function renderGame() {
        ...
        allBoxes.forEach((box, index) => {
            ...
            if (box.hp <= 0) {
                ...
                const y = Math.random() * GH;
                const area = Math.random() * 150;
                const box = new CollisionBox(x, y, 5 + area, 5 + (150 - area), Math.round(Math.min(360, Math.random() * allBalls.length * 10)));
        ...
        if (allBalls.length > 0) {
            ...
            if (sorted[0].y > playerA.board.y + pad)
                playerA.board.vy += speed;
            if (sorted[0].y < playerA.board.y - pad)
                playerA.board.vy -= speed;
            if (sorted[last].y > playerB.board.y + pad)
                playerB.board.vy += speed;
            if (sorted[last].y < playerB.board.y - pad)
                playerB.board.vy -= speed;
            playerA.board.x = 15;
        ...
        allBalls.forEach((ball, index) => {
            ...
            if (ball.y < 0 + ball.size / 2)
                ball.vy = Math.abs(ball.vy * ball.acc); // top
            if (ball.y > GH - ball.size / 2)
                ball.vy = -Math.abs(ball.vy * ball.acc); // bottom
            ball.render(ctx);
        ...
        if (allBalls.length <= Math.round((playerA.score + playerB.score) / 5 + 3)) {
